...
static std::vector<uint8_t> makeBeacon(uint8_t ssidTag, const std::string& ssid,
...
TEST_CASE("Beacon: parse SSID and BSSID") {
    ...
    auto b = wlan::parseBeacon(buf.data(), buf.size(), pcapng::kLinkTypeIeee80211);
    REQUIRE(b.has_value());
...
TEST_CASE("Non-beacon frame returns nullopt") {
    ...
    f[0] = 0x40;
    auto b = wlan::parseBeacon(f.data(), f.size(), pcapng::kLinkTypeIeee80211);
