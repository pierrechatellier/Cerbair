 ...
 *
 * Cet IE est extrait d'un beacon 802.11. L'OUI et le type identifient le protocole constructeur ;
 * le payload contient les donnees a decoder.
 */
 ...
 *
 * Un beacon est un type particulier de trame de gestion 802.11, emis periodiquement par un point
 * d'acces. Il ne represente donc pas n'importe quelle trame Wi-Fi.
 */
struct Beacon {
 ...
 *
 * Une trame peut etre une trame 802.11 directe ou etre precedee d'un en-tete Radiotap. Les autres
 * types de trames 802.11, ainsi que les autres types de liaison, ne sont pas retournes.
 *
 ...
 * @return Le beacon extrait, ou std::nullopt si le paquet n'est pas un beacon exploitable.
 */
std::optional<Beacon> parseBeacon(const uint8_t* data, size_t len, uint16_t linkType);
