...
void PcapNgReader::readExact(void* dst, size_t n, const char* what) {
    // Une lecture partielle rendrait impossible l'interpretation fiable du bloc ; elle est donc
    // transformee en erreur explicite.
...
bool PcapNgReader::next(Packet& out) {
    while (true) {
        // Les blocs inconnus sont lus et valides avant d'etre ignores ; cela maintient la
        // synchronisation avec le bloc PCAPNG suivant.
        uint8_t hdr[8];
        ...
        const bool isShb = hdr[0] == 0x0A && hdr[1] == 0x0D && hdr[2] == 0x0D && hdr[3] == 0x0A;

        uint32_t blockType = 0;
        uint32_t totalLen = 0;

        if (isShb) {
            // Le SHB est necessaire pour connaitre l'endianness des longueurs de blocs qui suivent.
            uint8_t magic[4];
            ...
            if (totalLen < 16 || (totalLen & 3)) throw ParseError("pcapng: invalid SHB length");
        ...
        if (!byteOrderSeen_) throw ParseError("pcapng: block before Section Header Block");

        blockType = rdU32(hdr, littleEndian_);
        totalLen = rdU32(hdr + 4, littleEndian_);
        if (totalLen < 12 || (totalLen & 3)) throw ParseError("pcapng: invalid block length");

        std::vector<uint8_t> body(totalLen - 12);
        if (!body.empty()) readExact(body.data(), body.size(), "block body");
        uint8_t trailer[4];
        ...
        switch (blockType) {
            case kInterfaceDescriptionBlock: {
                // L'IDB associe un type de liaison a l'identifiant d'interface reutilise ensuite
                // par les blocs de paquets.
            ...
            case kEnhancedPacketBlock: {
                // L'EPB fournit l'interface, l'horodatage et la longueur capturee avant les octets
                // bruts du paquet.
                ...
                if (body.size() < 20 + capLen) throw ParseError("pcapng: EPB payload truncated");
            ...
            case kSimplePacketBlock: {
                // Un SPB ne porte pas d'interface ni d'horodatage : le lecteur utilise donc la
                // premiere interface declaree.
